...
class RobotP3DX(Robot):
    ...
    def move(self, v: float, w: float) -> None:
        ...
        # 1) calculate the linear velocity of each wheel
        b = self._track / 2
        ang_left = (v - b * w) / self._wheel_radius
        ang_right = (v + b * w) / self._wheel_radius

        # 2) send to robot
        self._sim.setJointTargetVelocity(self._motors["left"], ang_left)
        self._sim.setJointTargetVelocity(self._motors["right"], ang_right)
    ...
    def _sense_encoders(self) -> tuple[float, float]:
        ...
        ang_left = encoders["left"] / self._dt
        ang_right = encoders["right"] / self._dt

        # TODO: 1.3. Solve forward differential kinematics (i.e., calculate z_v and z_w).
        z_v = 0.0
        z_w = 0.0

        z_v = ((ang_right + ang_left) / 2) * self._wheel_radius

        z_w = ((ang_right - ang_left) / self._track) * self._wheel_radius
